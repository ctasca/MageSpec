 ...
 */
class SuiteHelperLocator implements ResourceLocatorInterface, SuiteLocatorInterface
{
    const LOCAL_CODE_POOL = 'local';

    const CLASS_TYPE = 'Helper';

    const VALIDATOR = '/^(helper):([a-zA-Z0-9]+)_([a-zA-Z0-9]+)_([a-zA-Z0-9]+)\/([a-z0-9]+)(_[\w]+)?$/';
    ...
    {
        if (strpos($file, 'Helper') > 0 && $this->isSuiteLocator($file)) {
            return true;
        }
