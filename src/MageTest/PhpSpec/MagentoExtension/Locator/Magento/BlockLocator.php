 ...
 */
class BlockLocator implements ResourceLocatorInterface, SuiteLocatorInterface
{
    const LOCAL_CODE_POOL = 'local';

    const CLASS_TYPE = 'Block';

    const VALIDATOR = '/^(block):([a-zA-Z0-9]+)_([a-zA-Z0-9]+)\/([a-z0-9]+)(_[\w]+)?$/';
    ...
    {
        if (strpos($file, 'Block') > 0 && !$this->isSuiteLocator($file)) {
            return true;
        }
